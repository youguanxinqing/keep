...
A bounded queue serializes process output and can tee a process's stdout and stderr,
in received order, into one append-only local file before it writes the prefixed
console line. A process may disable its console destination only when a log directory
is configured; runtime log failures fall back to console output. The queue is drained
before `keep` exits. A second terminal controls the supervisor through its Unix socket.
